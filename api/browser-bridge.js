...
export default async function handler(request, response) {
  ...
  const events = incoming.flatMap((item) => {
    ...
    return [{
      ...
      message_type: item.verified_unread_preview ? 'verified_unread_preview' : item.unread_notice ? 'unread_notice' : 'text',
      ...
      raw_payload: { source: 'authorized-whatsapp-web-chat', source_message_key: String(item.source_message_key || '').slice(0, 1000), verified_unread_preview: Boolean(item.verified_unread_preview), unread_count: Number(item.unread_count || 0) },
    }];
  });
