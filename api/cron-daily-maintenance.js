...
export default async function handler(request, response) {
  ...
  // usan las funciones puras de daily-maintenance.mjs) sigue disponible para
  // debugging fino, pero nadie sin contexto del código puede leerlo de un
  // vistazo. `summary` agrega esos mismos números en un solo bloque con
  ...
  return response.status(200).json({
    ...
    tasksClosed,
    summary: {
      descripcion: `Se cerraron ${tasksClosed} tarea(s) vencida(s) y se crearon ${autoFollowupTasksCreated} tarea(s) automática(s) de seguimiento en ${results.length} workspace(s)${failed.length ? `, ${failed.length} con error` : ''}.`,
