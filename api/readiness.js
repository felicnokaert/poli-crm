...
export default async function handler(request, response) {
  ...
  const [workspaceResult, eventsResult] = await Promise.all([
    ...
    fetch(`${process.env.SUPABASE_URL}/rest/v1/whatsapp_events?select=channel,direction,occurred_at&direction=eq.inbound&order=occurred_at.desc&limit=500`, { headers }),
  ]);
  ...
  for (const key of ['general', 'penosil']) {
    ...
    channels[key] = {
      configured: Boolean(key === 'general' ? process.env.WHATSAPP_GENERAL_PHONE_ID : process.env.WHATSAPP_PENOSIL_PHONE_ID),
