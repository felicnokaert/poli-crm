...
function DataSettings({ data, setData, session, syncStatus }) {
  ...
  async function importClients(event) {
    ...
    try {
      ...
      setMessage(`Vista previa lista: ${result.added} empresas nuevas, ${result.updated} actualizadas y ${result.skipped} filas omitidas. Todavía no se guardó nada.`);
    } catch (error) {
  ...
  function confirmClientImport() {
    ...
    const duplicateNote = pendingClientImport.duplicatePhones?.length ? ` ${pendingClientImport.duplicatePhones.length} teléfonos compartidos quedaron señalados para revisión, sin fusionarse.` : '';
    setMessage(`CSV incorporado: ${pendingClientImport.added} empresas nuevas, ${pendingClientImport.updated} actualizadas y ${pendingClientImport.skipped} filas omitidas.${duplicateNote}`);
