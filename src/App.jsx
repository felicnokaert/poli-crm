...
function Clients({ clients, query, setQuery, onOpenClient }) {
  return <section className="panel"><div className="panel-head"><div><span className="eyebrow">Cartera unificada · {clients.length} visibles</span><h2>Clientes y prospectos</h2></div><label className="search"><Search size={17}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Empresa, CUIT, contacto, teléfono…" /></label></div>{clients.length ? <div className="client-table">{clients.map((client) => <button className="client-row" onClick={() => onOpenClient(client.id)} key={client.id}><div className="avatar">{client.company.slice(0, 2).toUpperCase()}</div><div><strong>{client.company}</strong><span>{client.contact || client.phone || client.email || 'Datos de contacto pendientes'}</span></div><span>{client.family}</span><span className={`temp ${(client.temperature || 'Tibio').toLowerCase()}`}>{client.temperature || 'Tibio'}</span><strong>{client.stage || 'Nuevo'}</strong></button>)}</div> : <Empty text="No hay clientes que coincidan con la búsqueda." />}</section>;
}
