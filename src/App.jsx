...
import { buildCommercialCohort } from './commercial-cohort';
...
function DataSettings({ data, setData, session, syncStatus }) {
  ...
  function importCommercialCohort() {
    ...
    const existingNames = new Set(data.clients.map((item) => normalize(item.company)));
    ...
    setData((current) => ({ ...current, clients: [...current.clients, ...newClients], tasks: [...current.tasks, ...newTasks] }));
    setMessage(`Cohorte comercial preparada: ${newClients.length} clientes y ${newTasks.length} tareas nuevas. ${cohort.clients.length - newClients.length} cuentas existentes fueron preservadas sin cambios.`);
  }
